...
nomCible=`nbtscan $cible | grep "server" | awk {'print $2'}`

if [ -z "$nomCible" ]
then
	nomCible=`nbtscan -v $cible | grep "<" | sed -n '1p' | awk {'print $1'}`
fi

if [ -z "$nomCible" ]
then
	echo -e "
$RED$BOLD [-]$DEFAULT$CYAN Le nom de la machine $cible n'a pas pu être trouvé.$DEFAULT"
else 
	echo -e "
$GREEN$BOLD [+]$DEFAULT$CYAN Le nom de la machine $cible est :$DEFAULT $nomCible"
fi
