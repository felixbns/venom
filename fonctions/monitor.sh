...
echo -e "$MAGENTA [...]$CYAN Activation du mode monitor de votre carte Wifi... $DEFAULT"
...
if [ "$wlan0" = "wlan0mon" ];then
	echo -e "$GREEN$BOLD [+]$DEFAULT$CYAN Le mode monitor de votre carte Wifi a été activé !$DEFAULT"
else
	sleep 1
	echo -e "$RED$BOLD [-]$DEFAULT$CYAN L'activation du mode monitor de votre carte wifi a échoué !$DEFAULT"
fi
