...
echo -e -n "$MAGENTA [>]$CYAN Entrez l'IP ou l'URL de la machine ciblée : $DEFAULT"
read cible

echo -e "$BLUE [...]$DEFAULT$CYAN Chargement... $DEFAULT
"

scan=`nmap -A -T4 $cible`

echo -e "$GREEN$BOLD [+]$DEFAULT$CYAN Voici le résultat du scan pour la machine $cible : $DEFAULT
"
...
echo -e -n "
$MAGENTA [>]$CYAN Voulez-vous connaître l'état d'un port en particulier ? (o/n) : $DEFAULT"
read ouinon


case $ouinon in
  	o) {			
			echo -e -n "$MAGENTA [>]$CYAN Entrez le(s) port(s) à scanner : $DEFAULT"
			read port

			echo -e "$BLUE [...]$DEFAULT$CYAN Chargement... $DEFAULT
			"

			scanPort=`nmap -Pn -n -p $port $cible`
