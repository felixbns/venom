...
echo -e -n "$MAGENTA [>]$CYAN Entrez l'URL du site web à scanner (http(s)://exemple.com/): $DEFAULT"
read url

echo -e "$BLUE [...]$DEFAULT$CYAN Scan du site en cours... $DEFAULT
"

dirbuster -u $url -l /usr/share/dirb/wordlists/common.txt -t 99 > /dev/null 2>&1


echo -e "$GREEN$BOLD [+]$DEFAULT$CYAN Fin du scan du site $cible$DEFAULT"
