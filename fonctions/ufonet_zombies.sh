...
echo -e "$GREEN$BOLD [+]$DEFAULT$CYAN Les zombies disponibles pour l'attaque sont : $DEFAULT
"
sleep 1

cd && cd ufonet/botnet/ && cat zombies.txt

nombre=`cd && cd ufonet/botnet/ && wc -l zombies.txt | awk {'print $1'}`

echo ""
echo -e "$GREEN$BOLD [+]$DEFAULT$CYAN Vous avez $nombre zombies ! $DEFAULT
"
